...
def read_network_distance(distance_matrix):
    """
    ...
    * Lines that start with '#' will be ignored
    ...
    # 初始化一个空字典
    protein_distances = {}
    #distance = pd.read_csv('DatasetS2_shortest_distance.txt',sep = "\t",header = None,names = ["proteinA","proteinB","distance"])
    distance = pd.read_csv(distance_matrix,sep = "\t",header = None,names = ["proteinA","proteinB","distance"],dtype = {"proteinA":str,"proteinB":str,"distance":int})
    distance_index = distance.set_index(['proteinA', 'proteinB'])
    protein_distances = distance_index['distance'].to_dict()
...
def single_proximity(sample):
    ...
    d, z, (mean, sd), pval = wrappers.query_proximity(distance,network, nodes_from, nodes_to, n_random=nsims, min_bin_size = 100)
    #print (('d=%s, z=%s, (mean, sd)=%s')%(d, z, (mean, sd)))

    # write in file
    current_directory = os.getcwd()
    final_directory = os.path.join(current_directory, outdir)
    ...
    with open(os.path.join(final_directory, filename), 'w') as f:
        ...
        f.write(drug_key +'\t'+ disease_save +'\t'+ str(d) +'\t' + str(z) +'\t'+ str(mean) +'\t'+ str(sd) + '\t' + str(pval) +'\n')
...
parser.add_argument("-dis", "--distance", help = "distance_matrix file")
parser.add_argument("-o", "--outdir", help = "path to save file")
...
distance = read_network_distance(distance_matrix)
...
for i in drug_targets.keys():
    if len(drug_targets[i]) >= 2:
        for j in disease_genes.keys():
            samples.append([i,j,drug_targets,disease_genes,G1,nsims,distance,outdir])
print ('number of samples:',len(samples))

# set up parallel computing
total_task = len(samples)
...
#    thread = threading.Thread(target=single_proximity, args=(params,))
